<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex-shrink-0 flex items-center">
                <a href="{{ route('dashboard') }}">
                    <img src="{{ asset('logo Assistance Client Plus.png') }}" alt="Assistance Client Plus"
                        class="h-24 w-auto">
                </a>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:space-x-8">
                ...
                @auth
                    @if (auth()->user()->role === 'admin')
                        <x-nav-link :href="route('commercials.index')" :active="request()->routeIs('commercials.*')">
                            {{ __('Commercials') }}
                        </x-nav-link>
                    @endif
                @endauth
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('tickets.index')" :active="request()->routeIs('tickets.*')">{{ __('Tickets') }}</x-responsive-nav-link>
            @auth
                @if (auth()->user()->role === 'admin')
                    <x-responsive-nav-link :href="route('commercials.index')" :active="request()->routeIs('commercials.*')">
                        {{ __('Commercials') }}
